...
export class CloudflareVectorStore implements VectorStore<CloudflareVectorFilter> {
  ...
  async query(
    ...
    if (options.mode.type === "keyword") {
      // Keyword mode - use the text query
      queryText = options.mode.text;
    } else if (options.mode.type === "hybrid") {
      // Hybrid mode - use the text query (vector search happens server-side)
      queryText = options.mode.text;
    } else if (options.mode.type === "semantic") {
    ...
    const results: VectorStoreQueryResponse = response.sources.slice(0, options.topK ?? 10).map((source) => {
      // Transform metadata to VectorStoreMetadata format
      const transformedMetadata: VectorStoreMetadata = {};
      if (source.metadata && typeof source.metadata === 'object') {
        for (const [key, value] of Object.entries(source.metadata)) {
          if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') {
            transformedMetadata[key] = value;
          } else if (Array.isArray(value) && value.every(v => typeof v === 'string')) {
            transformedMetadata[key] = value as string[];
          }
          // Skip values that don't match VectorStoreMetadata types
        }
      }
